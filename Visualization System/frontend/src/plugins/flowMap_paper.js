import { dividePoints, dividePoints2 } from "./kruskal";
...
function mergeAll(source, targets, alpha, log = true) {
    ...
    function merge(source, targets, alpha, direction=null, split=false, split_bias=0) {
        ...
        if (totalAngle >= Math.PI*2/3 || split) {
            let tot_flow = 0;
            for(let target of targets) {
                tot_flow += target.flow;
            }

            let result = dividePoints2([tmp_source].concat(targets));
            console.log("div", result);
            let means = [];
            for(let key in result) {
                let tmp_mean = mean(result[key]);
                tmp_mean.key = key;
                means.push(tmp_mean);
            }
            ...
            let ret = [];
            let now_flow = 0;
            means = sortPoints(tmp_source, means);
            console.log("div means", means);
            for(let tmp_mean of means) {
                let tmp_flow = 0;
                for (let target of result[tmp_mean.key]) {
                    tmp_flow += target.flow;
                }
                let other_flow = tot_flow - now_flow - tmp_flow;
                ret = ret.concat(merge(source, result[tmp_mean.key], alpha, direction, false, split_bias + now_flow/2 - other_flow/2));
                now_flow += tmp_flow;
            }
