...
const GridLayout = function(parent) {
  ...
  that.update_info_from_parent = function() {
    ...
    that.layout_height = that.parent.svg_height - that.grid_margin - 8;
...
//    console.log("global_palette", palette_all, summary_res);
  ...
  that.update_layout = function(grid_info, color_set = null) {
    ...
    that.cell = Math.min(
      ...
      that.layout_height / that.size[1]
    );
    ...
    that.grid_margin_y =
      Math.min((that.layout_height - that.grid_height) / 2 + that.grid_margin + 8, 36);
    let meta = {
...
//    console.log("show thresholdValue", sheld);
    ...
    for(let i=0;i<max_conf.length;i++)if_confuse.push(false);
...
//          }
    ...
    for(let i=conf_argsort.length-1;i>=0;i--){
      ...
      if(show_images.length*that.parent.min_image_size2*that.parent.min_image_size2>conf_argsort.length*that.cell*that.cell*0.33)break;
    }
...
//      grid.show_image = ((that.parent.render_image && grid_info.grid.length <= 400 && (x%3===0) && (y%3===0)) ? true : false);
      ...
      grid.o_show_image = if_show_images[d];
      grid.image_bias = [image_bias[d][1], image_bias[d][0]];
      grid.image_ratio = grid_info["image_ratio"][d];
...
//      grid.confuse_label_name = grid_info.label_names[grid.confuse_label];
      ...
      if (!(grid.label in cclasses)) {
        cclasses[grid.label] = [cid, grid.color];
        cid++;
      }
      grid.cclass = cclasses[grid.label][0];
