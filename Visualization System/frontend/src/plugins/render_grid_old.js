...
const GridRender = function(parent) {
  ...
  that.update_detections = function(sample_id, item) {
    // console.log("update grid", sample_id, item.category);
    console.log("grid render update detections", sample_id, item, that.detection_items[sample_id]);
    that.grids.forEach(grid => {
      if (grid.sample_id == sample_id) {
        grid.detection_category = item.category;
      }
    });
...
//    console.log("render", that.meta);
    ...
    that.boundary_group.attr(
      "transform",
      `translate(${that.meta.delta_x}, ${that.meta.delta_y})`
    );
    that.highlight_group.attr(
      "transform",
      `translate(${that.meta.delta_x}, ${that.meta.delta_y})`
    );
    that.init_recall();
...
//      .attr("height", d => d.height - 2 * that.image_border)
  ...
  that.click_render_images = function(boxes) {
    ...
    that.e_grids2
      ...
      .transition()
      .duration(that.update_ani)
      .attr("opacity", d => (that.parent.show_images && d.show_image && d.use_image_bias) ? 1 : 0)
      .on("end", function() {
        d3.select(this).style("visibility", d => (that.parent.show_images && d.show_image) ? "visible" : "hidden")
      });
...
//      .attr("fill", e => (!that.one_partition && !that.is_zoomout) ? `rgba(${light_bias+e.pcolor[0]*light_rate},${light_bias+e.pcolor[1]*light_rate},${light_bias+e.pcolor[2]*light_rate},${e.pcolor[3]})` : `rgba(${light_bias+e.color[0]*light_rate},${light_bias+e.color[1]*light_rate},${light_bias+e.color[2]*light_rate},${e.color[3]})`)
  ...
  that.update = function() {
    ...
    let highlight = that.svg.select("rect.highlight_rect")
    let tmp_width = highlight.attr("width");
    let tmp_height = highlight.attr("height");
    let tmp_x = highlight.attr("x");
    let tmp_y = highlight.attr("y");
    let find_flag = false;
    ...
    highlight
      ...
      .attr("fill", "none")
      .attr("stroke", "black")
      .attr("stroke-width", 8*that.layout.stroke_width)
      .on("end", function() {
        d3.select(this)
          .style("visibility", find_flag ? "visible" : "hidden");
      });
