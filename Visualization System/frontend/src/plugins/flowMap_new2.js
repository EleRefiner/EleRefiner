...
function getMinDist(source, targets) {
    let dist = 10000000;
    for (let i = 0; i < targets.length; i++) {
        let tmp_dist = distance(source, targets[i]);
        if (tmp_dist <= dist) {
            dist = tmp_dist;
        }
    }
...
function mergeAll(source, targets, alpha, log = false) {
    ...
    function merge(source, targets, alpha, direction=null, split=false, split_bias=0, bias_consist=true) {
        if (log) {
            console.log("source", source)
            console.log("targets", targets)
            console.log("bias", split_bias, bias_consist)
        }
        if (targets.length === 0) {
            return []
        }
        ...
        let totalAngle = angleClockDifference(angle(tmp_source, targets[0]), angle(tmp_source, targets[targets.length - 1]))
        let minDist = getMinDist(tmp_source, targets);
        // console.log("init", 0, totalAngle, minDist);
        if (((totalAngle < Math.PI/500)||(minDist<10)) && (direction != null) && (bias_consist)) {
        // if (((totalAngle < Math.PI/50)||(minDist<30)) && (direction != null)) {
            let ori_split_bias = split_bias;
            let new_split_bias = split_bias;
            let tmp_d = (split_bias>=0) ? 1 : -1;
            let tmp_l = 0, tmp_r = Math.max(0, 20-Math.abs(ori_split_bias));
            for(let i=0;i<5;i++) {
                let tmp_mid = (tmp_l + tmp_r) / 2;
                new_split_bias = ori_split_bias + tmp_mid * tmp_d;
                let line_direction = [-direction.y, direction.x];
                let new_tmp_source = {};
                new_tmp_source.x = source.x + new_split_bias * line_direction[0] * stroke_width;
                new_tmp_source.y = source.y + new_split_bias * line_direction[1] * stroke_width;
                // console.log("move0", new_tmp_source);
                let new_targets = sortPoints(new_tmp_source, targets)
                totalAngle = angleClockDifference(angle(new_tmp_source, new_targets[0]), angle(new_tmp_source, new_targets[new_targets.length - 1]))
                minDist = getMinDist(new_tmp_source, new_targets);
                // console.log("move", tmp_mid, totalAngle, minDist);
                // console.log("move2", new_tmp_source, new_targets);
                if ((totalAngle < Math.PI/500)||(minDist<10)) {
                    tmp_l = tmp_mid;
                    // tmp_source.x = new_tmp_source.x;
                    // tmp_source.y = new_tmp_source.y;
                    // split_bias = new_split_bias;
                    // targets = new_targets;
                }
                else {
                    tmp_r = tmp_mid;
        ...
        if (hasNearbyTarget) {
            ...
            const paths = merge(source, filteredTargets, alpha, direction, false, split_bias, bias_consist)
            const ret = []
        ...
        if (totalAngle >= alpha || split) {
            ...
            let bias_consist1 = bias_consist, bias_consist2 = bias_consist;
            if ((Math.abs(split_bias)>=0.5)&&((split_bias)*(-tot_flow2/2)<0)) {
                bias_consist1 = false;
            }
            if ((Math.abs(split_bias)>=0.5)&&((split_bias)*(tot_flow1/2)<0)) {
                bias_consist2 = false;
            }
            let ret = merge(source, targets1, alpha, direction, false, split_bias-tot_flow2/2, bias_consist1).concat(merge(source, targets2, alpha, direction, false, split_bias+tot_flow1/2, bias_consist2));
            setBias(ret, direction, split_bias);
            return ret;
        }
        ...
        if (Math.abs(split_bias)<0.5) bias_consist = true;
        // if ((Math.exp(length/50)>100) && (Math.abs(split_bias)<0.5)) bias_consist = true;

        const paths = merge(midpoint, targets, alpha, new_direction, true, split_bias, bias_consist);
