...
class Port(object):
    def __init__(self, sample_num=(30, 30), info_dict=None) -> None:
    ...
    def update_sample_num(self, sample_num):
        self.data_ctrler.sample_num = sample_num
    ...
    def package_gridlayout(self, parent: int, index: int, grid: np.ndarray, grid_size, labels: np.ndarray, top_labels: np.ndarray, gt_labels: np.ndarray, part_labels: np.array, samples: np.ndarray, feature, top_part, info_before, zoomout=False):
        gridlayout = {}
        ...
        gridlayout["label_names"] = self.label_hierarchy.label_hierarchy['id2label']
        gridlayout["size"] = grid_size
        gridlayout["sample_ids"] = samples
    ...
    def top_gridlayout(self) -> dict:
        self.stack = [-1]
        # self.color_stack = [None]
        grids, grid_size, labels, top_labels, gt_labels, part_labels, samples, features, top_part, info_before = self.data_ctrler.getTopLayout()
        self.stack.append(0)        
        return self.package_gridlayout(-1, 0, grids, grid_size, labels, top_labels, gt_labels, part_labels, samples, features, top_part, info_before)
    
    def layer_gridlayout(self, node_id, samples, pre_sampled_id=None, zoom_without_expand=False, zoom_balance=False) -> dict:
        self.cur_idx += 1
        grids, grid_size, labels, top_labels, gt_labels, part_labels, samples, features, top_part, info_before = self.data_ctrler.gridZoomIn(samples, zoom_without_expand=zoom_without_expand, zoom_balance=zoom_balance)
        self.stack.append(self.cur_idx)
        return self.package_gridlayout(node_id, self.cur_idx, grids, grid_size, labels, top_labels, gt_labels, part_labels, samples, features, top_part, info_before)
    
    def zoom_out_gridlayout(self, node_id) -> dict:
        cur_id = self.stack.pop()
        # self.color_stack.pop()
        assert cur_id == node_id and node_id > 0
        grids, grid_size, labels, top_labels, gt_labels, part_labels, samples, features, top_part, info_before = self.data_ctrler.gridZoomOut()
        return self.package_gridlayout(self.stack[-2], self.stack[-1], grids, grid_size, labels, top_labels, gt_labels, part_labels, samples, features, top_part, info_before, True)
