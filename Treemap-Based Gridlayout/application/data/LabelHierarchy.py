...
class LabelHierarchy(object):
    ...
    def load(self, dataset='imagenet'):
        ...
        if dataset == "infographic":
            self.features = np.load(os.path.join(path, 'infographic_features.npy'))
            self.labels = np.load(os.path.join(path, 'infographic_labels.npy'))
            self.gt_labels = np.load(os.path.join(path, 'infographic_labels.npy'))
            self.label_hierarchy = self._load_json(os.path.join(path, 'infographic.json'))
